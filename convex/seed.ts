...
export const seedDatabase = mutation({
  ...
  handler: async (ctx) => {
    ...
    for (const item of existingExperiences) await ctx.db.delete(item._id);
    for (const item of existingFaqs) await ctx.db.delete(item._id);
    for (const item of existingReviews) await ctx.db.delete(item._id);
    for (const item of existingSubcategories) await ctx.db.delete(item._id);
    for (const item of existingCategories) await ctx.db.delete(item._id);
    for (const item of existingCities) await ctx.db.delete(item._id);
    for (const item of existingUsers) await ctx.db.delete(item._id);
    ...
    for (const city of CITIES) {
      for (const categoryName of CATEGORIES) {
        const subcategories = SUBCATEGORIES_BY_CATEGORY[categoryName as keyof typeof SUBCATEGORIES_BY_CATEGORY];
        ...
        for (let i = 0; i < 2; i++) {
          ...
          const experienceId = await ctx.db.insert("experience", {
            ...
            categoryId: categoryIds[categoryName] as any,
            subcategoryId: subcategoryIds[subcategoryName] as any,
            cityId: cityIds[city.cityName] as any
          });
