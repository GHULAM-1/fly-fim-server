...
exports.getWorldwideData = exports.getWorldwideSubcategoryPageDataFiltered = exports.getWorldwideSubcategoryPageData = exports.getFilteredCategoryPageData = exports.getCategoryPageData = void 0;
...
const structureSimplifiedExperience = async (ctx, experience) => {
...
const structureFullExperienceWithImageUrls = async (ctx, experience) => {
    ...
    handler: async (ctx, args) => {
        ...
        for (const subcategoryId of subcategoryIds) {
            ...
            if (subcategoryDoc) {
                ...
                const structuredExperiences = await Promise.all(subcategoryExperiences.map(exp => structureDetailedExperience(exp)));
        ...
        const structuredTopExperiences = await Promise.all(topExperiences.map(exp => structureDetailedExperience(exp)));
        const structuredPopularExperiences = await Promise.all(popularExperiences.map(exp => structureDetailedExperience(exp)));
        ...
        const reviewsWithUrls = await Promise.all(allReviews.map((review) => structureReviewWithImageUrls(ctx, review)));
        ...
        const allTopExperiences = allExperiences.filter((exp) => exp.isTopExperience === true);
        ...
        const structuredAllCategories = await Promise.all(allCategoriesResult.map(async (category) => ({
            ...
            subcategories: await Promise.all(category.subcategories.map(async (subcategory) => ({
                subcategoryName: subcategory.subcategoryName,
                experiences: await Promise.all(subcategory.experiences.map((exp) => structureDetailedExperience(exp)))
            })))
        })));
...
exports.getFilteredCategoryPageData = (0, server_1.query)({
    args: {
        categoryId: values_1.v.id("category"),
        sortBy: values_1.v.optional(values_1.v.union(values_1.v.literal("popular"), values_1.v.literal("lowToHigh"), values_1.v.literal("highToLow")))
    },
    handler: async (ctx, args) => {
        // Helper function to structure experience with detailed sections (like experienceFunctions)
        const structureDetailedExperience = async (exp) => {
            ...
            switch (args.sortBy) {
                case "popular":
                    ...
                    experiences = experiences.sort((a, b) => {
                        const aPopular = a.tagOnCards?.toLowerCase().includes('popular') ? 1 : 0;
                        const bPopular = b.tagOnCards?.toLowerCase().includes('popular') ? 1 : 0;
...
exports.getWorldwideSubcategoryPageData = (0, server_1.query)({
    args: {
        categoryId: values_1.v.id("category"),
    ...
    handler: async (ctx, args) => {
        ...
        const subcategoryExperiences = experiences.filter(exp => exp.subcategoryId === subcategoryDoc._id);
        ...
        for (const subId of categorySubcategoryIds) {
            ...
            if (subDoc) {
                categorySubcategoryNames.push(subDoc.subcategoryName);
            }
        }
...
exports.getWorldwideSubcategoryPageDataFiltered = (0, server_1.query)({
    args: {
        categoryId: values_1.v.id("category"),
        subcategoryName: values_1.v.string(),
        sortBy: values_1.v.optional(values_1.v.union(values_1.v.literal("popular"), values_1.v.literal("lowToHigh"), values_1.v.literal("highToLow")))
    ...
    handler: async (ctx, args) => {
        ...
        let experiences = await ctx.db
            ...
            .collect();
        // Filter by subcategory
        let subcategoryExperiences = experiences.filter(exp => exp.subcategoryId === subcategoryDoc._id);
        // 3) Apply sorting if provided
        if (args.sortBy) {
            switch (args.sortBy) {
                case "popular":
...
exports.getWorldwideData = (0, server_1.query)({
    args: {},
    handler: async (ctx) => {
        // Get all experiences with simplified structure and image URLs
        const experiences = await ctx.db.query("experience").collect();
        const simplifiedExperiences = await Promise.all(experiences.map((experience) => structureSimplifiedExperience(ctx, experience)));
        // Get all categories
        ...
        const categoriesWithSubcategories = await Promise.all(categories.map(async (category) => {
            ...
            const usedSubcategoryIds = [...new Set(categoryExperiences.map(exp => exp.subcategoryId))];
            // Filter subcategories to only include those actually used
            const linkedSubcategories = subcategories.filter(sub => usedSubcategoryIds.includes(sub._id));
            return {
                ...category,
        ...
        const reviewsWithUrls = await Promise.all(reviews.map((review) => structureReviewWithImageUrls(ctx, review)));
...
//# sourceMappingURL=worldwideFunctions.js.map